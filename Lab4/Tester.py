...
def calculate_PSNR(gt_list, pred_list):
    total_psnr = 0
    n = len(gt_list)
    for gt, pred in zip(gt_list, pred_list):
        # 注意：Generate_PSNR 需要形狀一樣且tensor，確保都是float tensor且在同一device上
        total_psnr = Generate_PSNR(gt.unsqueeze(0).to(pred.device), pred.unsqueeze(0))
    return total_psnr 
...
class Test_model(VAE_Model):
    ...
    def val_one_step(self, img, label, idx=0):
        ...
        PSNR = calculate_PSNR(label_list, decoded_frame_list[1:])
        return generated_frame, PSNR
